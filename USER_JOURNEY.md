...
Swisslaw is an experimental, free browser tool. The intended experience is useful legal orientation in ordinary language, with transparent automatic public-source research. The current small-model implementation has not met a general legal-quality release standard.
...
2. **Choose or write.** The guided choices are local and instantaneous. The first complete route asks about ordinary employee resignation; back and editable breadcrumbs clear dependent choices. Public/unknown employment regime and other topics continue in free text. The overtime and marriage guides provide prepared, conditional explanations after source checks without downloading a model. The selected category remains above the free-text box. Other free questions start the selected model. First use needs about 1.1 GB for 2B or 2.4 GB for 4B, a compatible WebGPU device and sufficient memory. Download progress and subsequent preparation have separate messages. Stop remains available. Returning visits may reuse the cache.
...
4. **Research automatically.** The model proposes terms locally; only canonical public legal vocabulary is sent, and the terms remain visible. Prepared paths send fixed public article references. No intermediate search approval is required. An unrecognised query asks for clarification locally, without sending the raw question.
...
6. **Continue.** Add a detail or follow-up; a new search again uses bounded public terms. Stop cancels active work. Technical generation errors allow retry. An unsuccessful search invites more ordinary-language detail. The conversation has a bounded context; Start again begins a fresh question.
...
## Prepared guides and remaining coverage

The overtime and getting-married paths give conditional practical steps using authored text and complete source snapshots. They are not model-generated answers and have not had independent human legal/language review. The two reported lay questions route to those guides; rules, source text and limitations can be expanded. Back/change choices, clearing and unmounting invalidate active retrieval. Other topics still rely on experimental model interpretation; this change does not resolve their substantive quality.
